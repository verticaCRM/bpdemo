...
<section id="content" class="container" style="margin-top:80px">
    <div class="portfolio_group">
        <div class="article-page" style="display:inline-block;max-width:70%">
...
foreach ($portfolio AS $item){
	...
	$isaddressrequested=($addreqstatus == "Requested" || $addreqstatus == 'Released')?1:0;
	...
	?>
		<div id="listing-<?php echo $portfolioitem ->name_value_list->listing_frontend_id_c->value;?>" class="portfolioitem" style="margin-top:10px;border-top:2px dashed #666666">
			...
			// echo '<pre>'; print_r( $listingitem); echo '</pre>';
		?>
		...
		<?php endif; ?>
			...
			<br />
			<a href="<?php echo $permalink.'--'.$listingitem->id;?>" class="portfoliobutton" data-id="<?php echo $listingitem->id;?>" class="portfoliobutton"><?php _e('View Listing Details','bbcrm');?></a>
			<form method="post" style="display:inline">
				<input type="hidden" name="pid" value="<?php echo $portfolioitem->id;?>">
				<input type="hidden" name="action" value="hide">
				<input type="submit" value="Hide from Data Room" class="portfoliobutton">
			</form> 
		...
		<?php
			...
			// if($inportfolio && $portfoliolisting->c_release_status != "Requested" && !$is_requested && !$isaddressreleased)
			 ...
			 if( !$isaddressrequested ) { ?>
				<form method="post" style="display:inline">
					<input type="hidden" name="pid" value="<?php echo $portfolioitem->id; ?>">
					<input type="hidden" name="lid" value="<?php echo $listingitem->id; ?>">
					<input type="hidden" name="action" value="request">
					<input type="submit" value="Request Address" class="portfoliobutton">
				</form>  
			<?php } ?>
		</div><!--portfolioitem-->
