...
class SH_Metabox extends WPAlchemy_Taxonomy
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = __('[Development Mode] ', SH_NAME) . $this->title;
		}
	...
	function _setup()
	{
		...
		else
		{
			...
			if( isset( $_GET['tag_ID'] ) ) {echo '<tr><table class="form-table"><tr>';}
			echo '<div class="vp-metabox">';
			$this->_enview($fields);
			echo '</div>';
			if( isset( $_GET['tag_ID'] ) ) echo '</tr></table></tr>';
		}
	...
	function _render_repeating_group($group)
	{
		...
		$html .= '<a href="#" class="button button-large docopy-' . $name . '">'. __('Add More', SH_NAME) . ' ' . $group['title'] . '</a>';
		$html .= '</div>';
