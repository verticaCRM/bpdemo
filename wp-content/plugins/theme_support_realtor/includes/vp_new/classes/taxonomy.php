...
class WPAlchemy_Taxonomy
{
	...
	function _init()
	{
		...
		if( !isset( $_GET['taxonomy'] ) && !is_admin() ) return;
		//$this->types = array('category');
		//exit('tsdfsfs');

		add_action( 'admin_enqueue_scripts',  array( $this, 'vp_enqueue' ) );
		$this->vp_metabox_enqueue();
		...
		if ( true )//$this->can_output())
		{
			foreach ($this->types as $type) 
			{
				add_action( $type.'_add_form_fields', array( $this, '_setup' ), 10, 2 );
				add_action( $type.'_edit_form_fields', array( $this, '_setup' ), 10, 2 );
				//add_meta_box($this->id . '_metabox', $this->title, array($this, '_setup'), $type, $this->context, $this->priority);
			
				//add_action('save_post', array($this,'_save'));
				add_action( 'edited_'.$type, array( $this, '_save' ), 10, 2 );  
				add_action( 'create_'.$type, array( $this, '_save'), 10, 2 );
			}
...
printr($this->template);
	...
	function can_output()
	{
		...
		if (isset($tax) AND ! in_array($tax, $this->types))
		{
			$can_output = FALSE;
		}
	...
	function _meta($post_id = NULL, $internal = FALSE)
	{
		...
		$post_id = isset( $_GET['tag_ID'] ) ? $_GET['tag_ID'] : null;
		...
		$meta = get_option( $this->id.$post_id );
