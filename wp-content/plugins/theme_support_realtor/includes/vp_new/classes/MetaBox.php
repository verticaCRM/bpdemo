...
add_action('admin_head', array('WPAlchemy_MetaBox1', '_global_head'));

add_action('admin_footer', array('WPAlchemy_MetaBox1', '_global_foot'));
...
class WPAlchemy_MetaBox1
{
	...
	function WPAlchemy_MetaBox1($arr)
	{
	...
	function _init()
	{
		...
		if ( ! WPAlchemy_MetaBox1::_is_post() AND ! WPAlchemy_MetaBox1::_is_page()) return;
	...
	{
		if ('post' == WPAlchemy_MetaBox1::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == WPAlchemy_MetaBox1::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = WPAlchemy_MetaBox1::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = WPAlchemy_MetaBox1::_get_post_id();
	...
	static function _global_head()
	{
		...
		jQuery(function($)
		{
			...
			$(document).on('click', '[class*=docopy-]', function(e)
			{
				...
				var p = $(this).parents('.wpa_loop:first');
	...
	function _meta($post_id = NULL, $internal = FALSE)
	{
		...
		$this->meta = $meta;*/
		$theme_options = is_array(get_option(SH_NAME.'_theme_options')) ? get_option(SH_NAME.'_theme_options') : array();
		$meta = (array)sh_set( sh_set( $theme_options, $this->id ), $this->id );
		array_pop( $meta );
		$theme_options[$this->id][$this->id] = $meta;
		$this->meta = sh_set( $theme_options, $this->id );
	...
	function _save($post_id) 
	{
		...
		WPAlchemy_MetaBox1::clean($new_data);
	...
	static function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i])) 
				{
					WPAlchemy_MetaBox1::clean($arr[$i]);
