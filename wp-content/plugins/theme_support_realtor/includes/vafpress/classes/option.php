...
class VP_Option extends WPAlchemy_MetaBox
{
	...
	public function __construct(array $configs)
	{
		...
		extract($test = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', SH_NAME ),
			'menu_label'            => __( 'Vafpress Options', SH_NAME ),
			'priority'              => 10,
		), $configs));

		// check and set required configs
		if(isset($option_key)) $this->set_option_key($option_key);
		else throw new Exception(__( 'Option Key is required', SH_NAME ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', SH_NAME ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', SH_NAME ), 1);
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", SH_NAME), false);
	}
	...
	{//printr($_POST);
		...
		if($result['status'])
		{
			...
			//printr($this->get_options_set()->get_values());
			// get back options from set
			$opt = $option;//$this->get_options_set()->get_values();
			//printr($opt);
			...
			update_option( $this->get_option_key(), $opt );
			$result = array( "status"=>true,"code"=>2,"message"=>"Changes are saved" );
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", SH_NAME);
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", SH_NAME);
				}
			}
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", SH_NAME);	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access.", SH_NAME);
		}
		return $result;
	}
	...
	public function init_options()
	{
		...
		$db_options = is_array(get_option($this->get_option_key())) ? get_option($this->get_option_key()) : array();
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', SH_NAME ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', SH_NAME));
			...
			$restore_section->set_title(__('Restore Default', SH_NAME));
			...
			$exim_section->set_title(__('Export/Import', SH_NAME));
