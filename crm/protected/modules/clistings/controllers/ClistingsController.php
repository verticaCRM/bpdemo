...
class ClistingsController extends x2base {

    public $modelClass = 'Clistings';
...
//CVarDumper::dump($model);        
    ...
    public function actionCreate() {
        $model=new Clistings;
        $users=User::getNames();
        if(isset($_POST['Clistings'])) {
            $temp = $model->attributes;
            $model->setX2Fields($_POST['Clistings']);
    ...
    protected function performAjaxValidation($model) {
        if(isset($_POST['ajax']) && $_POST['ajax']==='clistings-form') {
    ...
    public function actionQtip($id) {
        $clisting = $this->loadModel($id);

        $this->renderPartial('qtip', array('clisting' => $clisting));
    }
