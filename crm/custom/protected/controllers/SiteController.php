        ...
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array(
                    ...
                    'mergeRecords', 'ajaxSave', 'listingsGalleryFix','dataRoomFiles'),
                'users' => array('@'),
            ),
            array('allow',
                'actions' => array('motd'),
...
public function actionShowListingFiles($listing_id, $portfolio_id, $buyer_id) {
    ...
    public function actionUpdatePortfolioItemStatus($porfolioId, $action_type) {

        $rel = X2Model::model('Portfolio')->findByAttributes(array('id' => $porfolioId));
        if (isset($rel)) {
            $rel->c_release_status = $action_type;
            if ($action_type == 'Released')
            {
                $admin_name = Yii::app()->user->name;

                $rel->c_date_released = strtotime(date("Y-m-d H:i:s"));
                $rel->c_released_by = $admin_name;
            }
            $rel->update();
        }
        if (isset($_GET['redirect'])) {
    ...
    public function actionShowHidePortfolioItem($porfolioId, $action_type) {
        $rel = X2Model::model('Portfolio')->findByAttributes(array('id' => $porfolioId));
        if (isset($rel)) {
            if ($action_type == 'hide')
            {
    ...
    /**
     * Add multiple records to record relationship
     *
     * A record can be a contact, opportunity, or account. This function is
     * called via ajax from the Buyers Portfolio Relationships Widget.
     *
     */
    public function actionAddPortfolioItems() {


        if(isset($_POST['checkedListingsValues']))
        {
            $checkedListingsValues = explode(',', $_POST['checkedListingsValues']);

            if(!empty($checkedListingsValues))
            {
                $modelName = $_POST['ModelName'];
                $modelId = $_POST['ModelId'];


                //get a list with all listings ID from portfolio that are already linked to this buyer
                $criteria=new CDbCriteria;
                $criteria->alias = 'Portfolio';
                $criteria->addCondition($criteria->alias.".c_buyer LIKE '%_".$modelId."'");
               // $criteria->addCondition("c_buyer LIKE '%_".$modelId."'");

                $buyerPortfolios = X2Model::model('Portfolio')->findAll($criteria);


                $allBuyersListings = array();
                if (!empty($buyerPortfolios))
                {
                    foreach ($buyerPortfolios as $Portfolio) {
                        $allBuyersListings [] = $Portfolio->c_listing_id;
                    }
                }



                //add / create portfolio / relations
                foreach ($checkedListingsValues as $listingID)
                {
                    if (empty($allBuyersListings) || !in_array($listingID, $allBuyersListings))
                    {


                        $listing = X2Model::model('Clistings')->findByPk($listingID);

                        $buyer = X2Model::model('Accounts')->findByPk($modelId);

                        if (!$buyer)
                        {
                            $buyer = X2Model::model('Contacts')->findByPk($modelId);
                        }
                        ...
                        $portfolioFields = array(
                            ...
                            'c_created_by_user' => $admin_name,
                            'c_added_from' => 'Backend',
                        );

                        $PortfolioModel->setX2Fields($portfolioFields);
                        $PortfolioModel->save ();
                        $newPortfolioItem = $PortfolioModel->id;

                        //create a relationship between Buyer and new created Portfolio Item
                        $relationshipModelName = "Portfolio";
                        $relationshipModelId = $newPortfolioItem;
                        $relationshipMutual = (!isset($relationshipDetails['mutual']))? false : $relationshipDetails['mutual'];
                        $relationshipFirstLabel = (!isset($relationshipDetails['firstLabel']))? '' : $relationshipDetails['firstLabel'];
                        $relationshipSecondLabel =(!isset($relationshipDetails['secondLabel']))? '' :  $relationshipDetails['secondLabel'];


                        $relationship = new Relationships;
                        $relationship->firstType = $modelName;
                        $relationship->firstId = $modelId;
                        $relationship->firstLabel = $relationshipFirstLabel;
                        $relationship->secondType = $relationshipModelName;
                        $relationship->secondId = $relationshipModelId;
                        $relationship->secondLabel = $relationshipSecondLabel;
            ...
            if ($validAccount) {
                ...
                if ($allValid && $c && $a && $o) { // all records created?
                    Relationships::create(
                            'Contacts', $contact->id, 'Accounts', $account->id
                    );
                    Relationships::create(
                            'Opportunity', $opportunity->id, 'Contacts', $contact->id
                    );
                    Relationships::create(
                            'Opportunity', $opportunity->id, 'Accounts', $account->id
                    );
...
//        $model->quoteAmount = Formatter::parseCurrency($model->quoteAmount,false);
    ...
    public function actionResolveDuplicates() {
        if (isset($_POST['action'], $_POST['ref'], $_POST['modelName'])) {
            ...
            if ($action === 'keepThis' && isset($_POST['data'])) {
            ...
            } /* x2prostart */ elseif ($action === 'mergeRecords' && isset($_POST['data'])) {
                $attributes = json_decode($_POST['data'], true);
                if ($ref == 'create') {
                    $model = new $modelName;
                    foreach ($attributes as $key => $value) {
                ...
                return;
            } /* x2proend */ elseif (empty($action) && isset($_POST['data'])) {
                $attributes = json_decode($_POST['data'], true);
                $model = X2Model::model($modelName)->findByPk($attributes['id']);
            }
    ...
    public function actionMergeRecords($modelName) {
        if (isset($_POST[$modelName])) {
            $data = $_POST[$modelName];
            $fields = X2Model::model($modelName)->getFields(true);
            $models = array();
            $model = new $modelName;
            foreach ($_GET['idArray'] as $id) {
                $models[$id] = X2Model::model($modelName)->findByPk($id);
                if (is_null($model->createDate) || $models[$id]->createDate < $model->createDate) {
                    $model->createDate = $models[$id]->createDate;
                }
            }
            ...
            if ($model->save()) {
                foreach ($models as $oldModel) {
                    $model->mergeRelatedRecords(
                        $oldModel, true, true, true, true, true, true, true);
                ...
                $this->redirect(array(strtolower(X2Model::getModuleName($modelName)) . '/view', 'id' => $model->id));
            }
        } else {
            if (isset($_GET['idArray'])) {
                if (count($_GET['idArray']) === 1) {

                }
                if (!Yii::app()->user->checkAccess(X2Model::getModuleName($modelName) . 'Update')) {
                    $this->denied();
                }
    ...
    /**
     * FIX Gallery Listing. For each existing listing check if gallery is set up properly
     */
    public function actionListingsGalleryFix() {
    	$sql =
            'SELECT x2_gallery_to_model.*, x2_gallery.id as main_gallery_id 
            FROM x2_gallery_to_model 
            LEFT JOIN x2_gallery ON (x2_gallery_to_model.id = x2_gallery.id) 
            ORDER BY x2_gallery_to_model.id DESC ';
        ...
        if (!empty($result))
        {
	        ...
	        {
		        if ($entry['main_gallery_id'] == '')
		        {
			        //add new entry on x2_gallery in order to fix the entries galleries
			        $sqlInsert = "INSERT INTO x2_gallery (`id`, `versions_data`, `name`, `description`) VALUES ('".$entry['id']."', 'a:1:{s:5:\"small\";a:1:{s:15:\"centeredpreview\";a:2:{i:0;i:98;i:1;i:98;}}}', '1', '1');";
			        $command = Yii::app()->db->createCommand($sqlInsert)->execute();
			        print_r('<pre>');print_r($sqlInsert);print_r('</pre>');		        
			    }
	        }
        }

		print_r('<pre>');print_r('actionListingsGalleryFix SUCCESSFULLY RUN');print_r('</pre>');	
